...
class Job(object):
	...
	def enqueue(self):
		...
		print('Render engine var = '+self.render_engine) #debugging
...
class RenderThread(object):
	...
	def create(self): 
		...
		print('Render engine detected in RenderThread().create() '+self.render_engine)#debugging
	...
	def sysinfohandler(): 
		while 1:
			try: #get state to check if window is still open
				...
				print ('window closed, terminating sysinfohandler loop') #debugging
				global systat_test
...
def check_missing_frames(index):
	...
	def nametext_confirm():
		...
		slider_left = ttk.Scale(ntframe, name='slider_left', from_=0, to=slidelength, orient=HORIZONTAL, length=slidelength*9, command=lambda x: get_slider())
		slider_left.grid(row=1, column=0, columnspan=3, sticky=W)
		slider_left.set(leftbreak)
		
		slider_right = ttk.Scale(ntframe, name='slider_right', from_=0, to=slidelength, orient=HORIZONTAL, length=slidelength*9, command=lambda x: get_slider())
...
#def enqueue_missing_frames():
#	frames_missing = getlist()
#
...
def check_frames_window():
	...
	checkjob.set(0) #set to 0 each time window opens.
